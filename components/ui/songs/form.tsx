...
import { createArtist, createSong, updateArtist, updateSong } from '@/app/lib/actions'; // Adjust the import according to your setup
...
export default function Form({ song, artists }: { song?: Song, artists: Artist[]}) {
  ...
  return (
    <form onSubmit={(e) => {
      ...
      <div className="rounded-md bg-surface-400 dark:bg-surface-dark-300 text-text dark:text-text-dark p-4 md:p-6 mt-8">
        ...
        <div className="mb-4">
          <label htmlFor="description" className="mb-2 block text-xl font-semibold">
            Descrição
          </label>
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <input
                ...
                className="peer block w-full rounded-md border border-surface-200 dark:border-surface-dark-500 bg-surface-600 dark:bg-surface-dark-300 py-2 pl-10 text-sm outline-2 placeholder:text-placeholder dark:placeholder:text-placeholder-dark"
                defaultValue={song?.title} ///CHANGE
                required
              />
